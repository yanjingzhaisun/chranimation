...
public class AMPropertyAction : AMAction {
    ...
    private Component cacheComponent; //only for string's delay function
	...
	public PropertyInfo propertyInfo{
		get {
			...
			cachedPropertyInfo = RCall.GetPropertyInfo(component.GetType(), propertyName);
	...
	public FieldInfo fieldInfo{
		get {
			...
			cachedFieldInfo = RCall.GetFieldInfo(component.GetType(), fieldName);
	...
	public MethodInfo methodInfo{
		get {
			...
			cachedMethodInfo = RCall.GetMethodInfo(component.GetType(), methodName, t);
    ...
    public string start_str = string.Empty;
    public string end_str = string.Empty;
	...
	public override void execute(int frameRate, float delay) {
		...
		if(fieldInfo != null) {
			if(hasCustomEase()) {
                ...
                if (valueType == (int)AMPropertyTrack.ValueType.String) CoroutineBehaviour.StartCoroutineDelay(component.gameObject, (x) => { fieldInfo.SetValue(component, start_str); }, getWaitTime(frameRate, delay));
			} else {
                ...
                if(valueType == (int)AMPropertyTrack.ValueType.String) CoroutineBehaviour.StartCoroutineDelay(component.gameObject, SetStringFunc, getWaitTime(frameRate, delay));
            }
			...
			if(hasCustomEase()) {
                ...
                if (valueType == (int)AMPropertyTrack.ValueType.String) CoroutineBehaviour.StartCoroutineDelay(component.gameObject, (x) => { propertyInfo.SetValue(component, start_str, null); }, getWaitTime(frameRate, delay));
            }
            else
            {
	...
	public bool targetsAreEqual() {
        ...
        if (valueType == (int)AMPropertyTrack.ValueType.String) return (start_str == end_str);
	...
	public object getStartValue() {
        ...
        if (valueType == (int)AMPropertyTrack.ValueType.String) return start_str; //return start_rect.ToString()+" -> "+end_rect.ToString();
        return "Unknown";
	}
	public object getEndValue() {
        ...
        if (valueType == (int)AMPropertyTrack.ValueType.String) return end_str; 
        return "Unknown";
	}
    ...
    public void SetStringFunc()
    {
        fieldInfo.SetValue(cacheComponent, start_str);
    }
}
