...
public class AMTake :ScriptableObject {
    ...
    [SerializeField]
    private string _name;					// take name
    public int frameRate = 30;				// frames per second
	public int numFrames = 120;			// number of frames
    ...
    // Add track to timeline action in AMTimeline (getInfoTextForAction/showFrames)
    ...
    // AMTimeline.buildAddTrackMenu / buildAddTrackMenu_Drag
    // prepare icon
    ...
    // add scale track
    public void addScaleTrack(GameObject obj) {
        AMScaleTrack a = ScriptableObject.CreateInstance<AMScaleTrack>();
        a.setName(getTrackCount());
        a.id = getUniqueTrackID();
        if (obj) a.obj = obj.transform;
        addTrack(a);
    ...
    // add animator track
    public void addAnimatorTrack(GameObject obj)
    {
        AMAnimatorTrack a = ScriptableObject.CreateInstance<AMAnimatorTrack>();
        a.setName(getTrackCount());
        a.id = getUniqueTrackID();
        if (obj && obj.GetComponent(typeof(Animator))) a.obj = obj.transform;
        addTrack(a);
    }
	...
	public void deleteTrack(int id, bool deleteFromGroup=true) {
        ...
        AMUtil.recordObject(this, "delete track");
		if(cameraSwitcher == trackValues[index]) cameraSwitcher = null;
        trackValues[index].destroy(); //delete track SO + all keys + actions
	...
	public List<int> getTrackIDsForGroup(int group_id, int start_id, int end_id, ref bool foundStartID, ref bool foundEndID) {
		List<int> track_ids = new List<int>();
    ...
    public void getTrackIDsForGroup(int group_id, List<int> track_ids)
    {
        AMGroup grp = getGroup(group_id);
        for (int i = 0; i < grp.elements.Count; ++i)
        {
            if (grp.elements[i] > 0)
                ...
                getTrackIDsForGroup(grp.elements[i], track_ids);
        }
    }
	...
	public void deleteGroup(int group_id, bool deleteContents) {
		...
		if(deleteContents) {
			...
			for(int k=0;k<elementGroup.elements.Count;k++) {
				if(elementGroup.elements[k] == group_id) {
                    ...
                    AMUtil.recordObject(elementGroup, "delete group");
        ...
        AMUtil.recordObject(this, "delete group");
		bool found = false;
		int j = 0;
	...
	public AMTrack getTrack(int id) {
		...
		if(index == -1 || index >= trackValues.Count) {
			Debug.LogError("Animator: Track id "+id+" not found.");
			return ScriptableObject.CreateInstance<AMTrack>();
		}
	...
	public void removeFromGroup(int source_id) {
		foreach(int id in rootGroup.elements) {
			if(id == source_id) {
                AMUtil.recordObject(rootGroup, "delete group");
		...
		foreach(AMGroup grp in groupValues) {
			foreach(int _id in grp.elements) {
				if(_id == source_id) {
                    AMUtil.recordObject(grp, "delete group");
	...
	public void deleteTrackFromGroups(int _id) {
		...
		foreach(int id in rootGroup.elements) {
			if(id == _id) {
                AMUtil.recordObject(rootGroup, "delete track from groups");
                rootGroup.elements.Remove(id);
				found = true;
				break;
			}
		...
		if (!found) {
			foreach(AMGroup grp in groupValues) {
				foreach(int id in grp.elements) {
					if(id == _id) {
                        AMUtil.recordObject(grp, "delete track from groups");
	...
	public void previewFrame(float _frame, bool orientationOnly = false, bool renderStill = true, bool skipCameraSwitcher = false, bool quickPreview = false /* do not preview properties to execute */) {
		...
		foreach(AMTrack track in trackValues) {
			...
			if(track is AMAudioTrack) continue;
				...
				else if(track is AMAnimationTrack) (track as AMAnimationTrack).previewFrame(_frame,frameRate);
                else if(track is AMAnimatorTrack) (track as AMAnimatorTrack).previewFrame(_frame, frameRate);
	...
	public void deleteKeysAfter(int frame) {
		...
		foreach(AMTrack track in trackValues) {
			...
			for(int i=0;i<track.keys.Count;i++) {
                var k = track.keys[i];
				if(k.frame > frame) {
                    AMUtil.recordObject(track, "delete keys");
                    // remove from list
	...
	public void deleteSelectedKeysFromTrack(int track_id) {
		...
		for(int i=0; i<track.keys.Count;i++) {
            ...
            AMUtil.recordObject(track, "delete selected keys");
            var k = track.keys[i];
            track.keys.RemoveAt(i);
            k.destroy();
    ...
    /// <summary>
    /// given the saved path, set the name of take
    /// </summary>
    public void setNameByProjPath(string newPath)
    {
	...
	public void offsetContextSelectionFramesBy(int offset) {
		...
		foreach(int track_id in contextSelectionTracks) {
			...
			foreach(AMKey key in _track.keys) {
				for(int i=0;i<contextSelection.Count;i+=2) {
					...
					if(contextSelection[i] <= key.frame && contextSelection[i+1] >= key.frame) {
                        ...
                        AMUtil.recordObject(key, "move key");
						key.frame += offset;
			...
			foreach( AMKey key in keysToDelete) {
                AMUtil.recordObject(_track, "move key deletion");
    ...
    public bool hasContextSelection()
    {
        return (contextSelection == null || contextSelection.Count > 0);
    }

    public bool hasContextSelectionTracks()
    {
        return (contextSelectionTracks == null || contextSelectionTracks.Count > 0);
    }
    ...
    public void rebind(RebindOption opt)
    {
    ...
    public void unbind(RebindOption opt)
    {
        for (int i = 0; i < trackValues.Count; ++i)
        {
            var track = trackValues[i];
    ...
    /// <summary>
    /// save the AMTake and subordinate AMTrack/AMGroup into ONE asset file
    /// the path is projectPath (starting at project root)
    /// </summary>
#if UNITY_EDITOR
    public void SaveAsset(AnimatorData mb)
    {
        string assetPath = AssetDatabase.GetAssetPath(this);
        while(string.IsNullOrEmpty(assetPath))
        { //not in assetDatabase yet
            assetPath = EditorUtility.SaveFilePanelInProject("Save Take", name, "asset", "save into assetDatabase");
        }
        
        SaveAsset(mb, assetPath);
    }
    public void SaveAsset(AnimatorData mb, string assetPath)
    {
        // create new asset if the take is not persistent yet, or forceUpdate if already persistent
        ...
        {
            string curPath = AssetDatabase.GetAssetPath(this);
            if (curPath != assetPath)
                Dbg.LogWarn("newly specified assetPath is different from current path, will save to current path: {0}", curPath);

            AssetDatabase.ImportAsset(curPath, ImportAssetOptions.ForceUpdate);
        }

        // save tracks
        if (trackValues != null)
        {
            foreach (var amTrack in trackValues)
            {
                amTrack.SaveAsset(mb, this);
            }
        }
        ...
        if (rootGroup != null)
        {
            rootGroup.SaveAsset(mb, this);
        }

        // save AMGroups
        if (groupValues != null)
        {
            foreach (var amGroup in groupValues)
            {
                amGroup.SaveAsset(mb, this);
            }
        }

        // save CameraSwitcher
        if (cameraSwitcher != null)
            cameraSwitcher.SaveAsset(mb, this);

        AssetDatabase.SaveAssets();
    }
#endif

    #endregion "assetdatabase"
    ...
    private void _destroy_undo()
    {
#if UNITY_EDITOR
        AMUtil.recordObject(this, "destroy tracks");
        var copyTrackLst = trackValues;
        trackValues = new List<AMTrack>();
        foreach (AMTrack track in copyTrackLst)
        {
            track.destroy();
        }

        AMUtil.recordObject(this, "destroy rootGroup");
        var copyRootGroup = rootGroup;
        rootGroup = null;
        copyRootGroup.destroy();

        if (groupValues != null)
        {
            AMUtil.recordObject(this, "destroy groups");
            var copyGroups = groupValues;
            groupValues = new List<AMGroup>();
            foreach (AMGroup grp in copyGroups)
            {
                grp.destroy();
            }
        }
