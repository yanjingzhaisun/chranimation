...
public class AMPropertyTrack : AMTrack {
	...
	public MethodInfo methodInfoMorphNames {
		get {
			...
			cachedMethodInfoMorphNames = RCall.GetMethodInfo( component.GetType(), "GetChannelNames");
			return cachedMethodInfo;
		}
	}
	...
	public void addKey(int _frame) {
        if (isValueTypeNumeric(valueType))
            ...
            addKey(_frame, getPropertyValueString());
    ...
    public void addKey(int _frame, string val)
    {
        foreach (AMPropertyKey key in keys)
        {
            // if key exists on frame, update key
	...
	public override void updateCache() {
		...
		for(int i=0;i<keys.Count;i++) {
				...
				if(methodInfoType == (int)ValueType.MorphChannels) {
                    ...
                    a.start_str = (keys[i] as AMPropertyKey).str;
                    if (a.endFrame != -1) a.end_str = (keys[i + 1] as AMPropertyKey).str;
				}else {
    ...
    public string getPropertyValueString()
    {
        // field
        if (fieldInfo != null)
            return (String)fieldInfo.GetValue(component);
        // property
        else
            return (String)propertyInfo.GetValue(component, null);
    }
	...
	public static bool isValidType(Type t) {
		if((t == typeof(string)) || (t == typeof(int)) || (t == typeof(long)) || (t == typeof(float)) || (t == typeof(double)) || (t == typeof(Vector2)) || (t == typeof(Vector3)) || (t == typeof(Color)) || (t == typeof(Rect)))
			return true;
		return false;
	}
	...
	public void previewFrame(float frame, bool quickPreview = false) {
		...
		foreach(AMPropertyAction action in cache) {
			...
			}else if(action.valueType == (int)ValueType.String) {
                if (fieldInfo != null) fieldInfo.SetValue(component, action.start_str);
                else if (propertyInfo != null) propertyInfo.SetValue(component, action.start_str, null);
