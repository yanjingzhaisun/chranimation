...
public class AnimatorData : MonoBehaviour {
    ...
    private RebindOption _rebindOption = new RebindOption();
    public RebindOption rebindOption
    {
        get
        {
            if (_rebindOption.rootTr == null)
                _rebindOption.rootTr = transform;

            return _rebindOption;
        }
    }
	...
	[HideInInspector] public int currentTake = -1;
	...
	public bool setCurrentTakeValue(int _take) {
		if(_take != currentTake) {
			...
			// change take
            evtBefChangeCurTake(getCurrentTake(), safeGetTake(_take));
    ...
    public AMTake safeGetTake(int idx)
    {
        if (idx < 0 || idx >= takes.Count)
            return null;
        return takes[idx];
    }
    ...
    public void removeCurrentTake()
    {
        if (currentTake >= 0)
        {
            removeTake(currentTake);
        }
    }

    public void removeTake(int index)
    {
//#if UNITY_EDITOR //we don't undo take level remove
//        if (!EditorApplication.isPlaying)
//            AMUtil.recordObject(this, "remove take");
//#endif
	...
	public void selectTake(int index) {
        if (currentTake != index)
        {
            evtBefChangeCurTake(safeGetTake(currentTake), safeGetTake(index));
            currentTake = index;
        }
	}
    ...
    //{
    //    AMUtil.recordObject(this, name);
    //    Undo.RecordObjects(takes, name);
    //}

    public const string BASEDIR = "Assets/Skele/AnimatorSP";

	#region event

    public delegate void BeforeChangeCurTake(AMTake curTake, AMTake newTake);
    public event BeforeChangeCurTake evtBefChangeCurTake;
