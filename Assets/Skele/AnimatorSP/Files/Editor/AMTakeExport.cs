...
public class AMTakeExport : EditorWindow {
	...
	void OnEnable() {
		window = this;
		this.titleContent = new GUIContent("Export Take" + (take == null ? "s" : ""));
	...
	bool saveSelectedItemsToScene()
	{
		...
		// save with changes
		MH.EUtil.SaveScene(saveScenePath,true);
		// restore scene
		MH.EUtil.OpenScene(saveScenePath);
