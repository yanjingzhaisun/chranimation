...
public class AnimatorTimeline : MonoBehaviour {
	...
	private static MethodInfo invoker{
		get {
			if(cachedInvoker == null) {
				cachedInvoker = RCall.GetMethodInfo(aData.GetType(), "Invoker");
			}
	...
	private static MethodInfo getMethodInfo(string goName, string componentName, string methodName, string[] typeNames) {
		...
		MethodInfo m;
		if(typeNames == null) m = RCall.GetMethodInfo(getCMP(goName,componentName).GetType(), methodName);
		else m = RCall.GetMethodInfo(getCMP(goName,componentName).GetType(), methodName,types.ToArray());
